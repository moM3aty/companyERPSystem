<?php
// app/views/helpdesk/index.php
$pageTitle = $data['title'] ?? 'خدمة العملاء والدعم الفني';
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sc-label { font-size: 12px; color: var(--text-muted); font-weight: 600; margin-bottom: 2px; }
        ...
        .tkt-num { font-family: monospace; font-size: 13px; font-weight: 700; color: var(--info-dark); background: var(--info-light); padding: 4px 10px; border-radius: 6px; direction: ltr; display: inline-block; }
        .tkt-subject { font-weight: 700; color: var(--text-dark); display: block; margin-bottom: 4px;}
        .tkt-client { font-size: 12px; color: var(--text-muted); display: flex; align-items: center; gap: 6px;}
        ...
        .pr-urgent { color: var(--danger); font-weight: 800; animation: blink 1.5s infinite; }
        @keyframes blink { 0%, 100% { opacity: 1; } 50% { opacity: 0.5; } }

        .act-btn { width: 34px; height: 34px; border-radius: 8px; border: 1px solid var(--border); background: transparent; display: inline-flex; align-items: center; justify-content: center; cursor: pointer; font-size: 13px; transition: all 0.2s; text-decoration: none; color: var(--text-body); }
        .act-btn.btn-view { color: var(--info); }
        .act-btn.btn-view:hover { background: var(--info-light); border-color: var(--info); }
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <!-- تم تعديل كود القائمة ليضيف رابط الدعم الفني مؤقتاً أو يمكن إضافته في الـ Layout -->
            ...
            <div style="display:flex;align-items:center;gap:16px;">
                ...
                <div>
                    ...
                    <div class="breadcrumb">
                        ...
                        <span>خدمة العملاء (Help Desk)</span>
                    </div>
                </div>
            </div>
        </header>
        ...
        <div class="page-body">
            ...
            <div class="summary-grid">
                <div class="summary-card">
                    <div class="sc-icon" style="background:var(--info-light);color:var(--info);"><i class="fas fa-ticket"></i></div>
                    <div>
                        <div class="sc-label">إجمالي التذاكر</div>
                        <div class="sc-value"><?php echo $stats->total ?? 0; ?></div>
                    </div>
                </div>
                <div class="summary-card">
                    <div class="sc-icon" style="background:var(--danger-light);color:var(--danger);"><i class="fas fa-envelope-open-text"></i></div>
                    <div>
                        <div class="sc-label">تذاكر مفتوحة</div>
                        <div class="sc-value" style="color:var(--danger);"><?php echo $stats->open_tickets ?? 0; ?></div>
                    </div>
                </div>
                <div class="summary-card">
                    <div class="sc-icon" style="background:var(--accent-light);color:var(--accent);"><i class="fas fa-fire"></i></div>
                    <div>
                        <div class="sc-label">تذاكر حرجة (SLA)</div>
                        <div class="sc-value" style="color:var(--accent);"><?php echo $stats->high_priority ?? 0; ?></div>
                    </div>
                </div>
            </div>
            ...
            <div class="toolbar">
                <div class="toolbar-right">
                    <div class="search-box">
                        <input type="text" id="searchInput" placeholder="ابحث برقم التذكرة أو العميل..." autocomplete="off">
                        <i class="fas fa-search"></i>
                    </div>
                </div>
                <div>
                    <a href="<?php echo URL_ROOT; ?>/helpdesk/create" class="btn-add">
                        <i class="fas fa-plus"></i> فتح تذكرة جديدة
                    </a>
                </div>
            </div>
                ...
                <div class="table-wrap">
                    <table>
                        ...
                        <tbody>
                            ...
                            ?>
                            <tr class="search-row" data-search="<?php echo htmlspecialchars($ticket->ticket_number . ' ' . $ticket->subject . ' ' . ($ticket->customer_name ?? '')); ?>">
                                <td><span class="tkt-num"><?php echo htmlspecialchars($ticket->ticket_number); ?></span></td>
                                <td>
                                    <span class="tkt-subject"><?php echo htmlspecialchars($ticket->subject); ?></span>
                                    <span class="tkt-client"><i class="fas fa-user-circle"></i> <?php echo htmlspecialchars($ticket->customer_name ?? 'غير محدد'); ?></span>
                                </td>
                                <td>
                                    <span class="<?php echo $prClass; ?>" style="font-size:12px;font-weight:700;"><i class="fas fa-flag" style="font-size:10px;margin-left:4px;"></i><?php echo $prLabel; ?></span>
                                </td>
                                <td>
                                    <div style="font-size:12px;color:var(--text-muted);"><i class="far fa-clock"></i> <?php echo date('Y-m-d H:i', strtotime($ticket->created_at)); ?></div>
                                </td>
                                <td>
                                    <span class="badge <?php echo $stClass; ?>"><?php echo $stLabel; ?></span>
                                </td>
                                <td>
                                    <span style="font-size:13px; font-weight:600;"><i class="fas fa-headset" style="color:var(--text-muted);margin-left:4px;"></i> <?php echo htmlspecialchars($ticket->assigned_name ?? 'غير مُعين'); ?></span>
                                </td>
                                <td>
                                    <a href="<?php echo URL_ROOT; ?>/helpdesk/show/<?php echo $ticket->id; ?>" class="act-btn btn-view" title="عرض ومعالجة"><i class="fas fa-eye"></i></a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                            ...
                            <tr>
                                <td colspan="7">
                                    <div class="empty-state">
                                        ...
                                        <h4>لا توجد تذاكر دعم فني</h4>
                                        <p>صندوق الوارد نظيف! لم يقم أي عميل بفتح تذكرة شكوى أو استفسار.</p>
                                    </div>
                                </td>
                            </tr>
                            <?php endif; ?>
