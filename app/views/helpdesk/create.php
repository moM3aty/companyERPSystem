<?php
// app/views/helpdesk/create.php
$pageTitle = $data['title'] ?? 'فتح تذكرة دعم فني';
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="page-body">
            ...
            <div class="form-header-card">
                <h2><i class="fas fa-ticket-alt" style="margin-left:8px;"></i> فتح تذكرة دعم فني جديدة</h2>
                <p>قم بتسجيل شكوى أو استفسار العميل ليتم متابعته من قبل فريق الدعم الفني بشكل منظم.</p>
            </div>

            <div class="form-card">
                <form action="<?php echo URL_ROOT; ?>/helpdesk/create" method="POST" id="ticketForm" novalidate>
                    
                    <div class="form-section">
                        <div class="form-section-title"><span class="fst-icon fst-teal"><i class="fas fa-info-circle"></i></span> معلومات التذكرة</div>
                        <div class="form-grid">
                            <div class="form-group full">
                                <label class="form-label">موضوع التذكرة <span class="req">*</span></label>
                                <input type="text" name="subject" class="form-input" id="tktSubject" placeholder="مثال: مشكلة في تسجيل الدخول للحساب" required>
                            </div>
                            <div class="form-group">
                                <label class="form-label">العميل المرتبط <span class="req">*</span></label>
                                <select name="customer_id" class="form-input" id="tktCustomer" required>
                            ...
                            <div class="form-group">
                                ...
                                <select name="priority" class="form-input">
                                    ...
                                    <option value="urgent">حرجة جداً (Urgent)</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        <div class="form-section-title"><span class="fst-icon fst-amber"><i class="fas fa-align-left"></i></span> تفاصيل الشكوى أو الطلب</div>
                        <div class="form-group full">
                            <textarea name="description" class="form-input" placeholder="اكتب وصفاً مفصلاً للمشكلة أو طلب العميل لتسهيل عملية المتابعة..." required></textarea>
                        </div>
                    </div>
                    
                    <div class="form-actions">
                        <button type="submit" class="btn-submit" id="btnSubmit"><i class="fas fa-paper-plane"></i> فتح التذكرة</button>
                        <a href="<?php echo URL_ROOT; ?>/helpdesk/index" class="btn-cancel">إلغاء</a>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        form.addEventListener('submit', function(e) {
            ...
            const customer = document.getElementById('tktCustomer');
            
            form.querySelectorAll('.form-input').forEach(el => el.style.borderColor = '');
            
            if (!subject.value.trim()) { subject.style.borderColor = 'var(--danger)'; valid = false; }
            if (!customer.value) { customer.style.borderColor = 'var(--danger)'; valid = false; }
