...
$pageTitle = $data['title'] ?? 'إضافة منتج جديد';
$categories = $data['categories'] ?? [];
$flash = $data['flash'] ?? null;
$currentUrl = 'product/create';
?>
<!DOCTYPE html>
...
<body>
    ...
    <div class="main-content">
            ...
            <div style="display:flex;align-items:center;gap:16px;">
                ...
                <div>
                    ...
                    <div class="breadcrumb">
                        ...
                        <a href="<?php echo URL_ROOT; ?>/product/index">إدارة المخزون</a>
                        <i class="fas fa-chevron-left" style="font-size:9px;"></i>
        ...
        <div class="page-body">
            ...
            <div class="form-header-card">
                <h2><i class="fas fa-box-open" style="margin-left:8px;"></i> إضافة منتج جديد للمستودع</h2>
                <p>أدخل بيانات المنتج الجديد بدقة ليتم استخدامه في الفواتير وتقارير الجرد</p>
            </div>
            ...
            <div class="form-card">
                <form action="<?php echo URL_ROOT; ?>/product/create" method="POST" id="productForm" novalidate>
                    ...
                    <div class="form-section">
                        <div class="form-section-title"><span class="fst-icon fst-teal"><i class="fas fa-cube"></i></span> البيانات الأساسية للمنتج</div>
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <div class="form-hint"><i class="fas fa-info-circle"></i> الاسم كما سيظهر في الفواتير للعملاء</div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="form-hint"><i class="fas fa-barcode"></i> رمز فريد يميّز المنتج — لا يمكن تكراره</div>
                            </div>
                        </div>
                    </div>

                    <div class="form-section">
                        <div class="form-section-title"><span class="fst-icon fst-amber"><i class="fas fa-layer-group"></i></span> التصنيف والمخزون</div>
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <div class="form-hint"><i class="fas fa-tags"></i> لسهولة فلترة وعرض المنتجات</div>
                            </div>
                            <div class="form-group">
                                <label class="form-label">الكمية الافتتاحية <span class="req">*</span></label>
                                <input type="number" name="quantity" class="form-input" id="prodQty" placeholder="0" min="0" required style="direction:ltr;text-align:right;">
                                <div class="form-hint"><i class="fas fa-boxes-stacked"></i> الكمية المتاحة حالياً في المستودع</div>
                            </div>
                        </div>
                    </div>

                    <div class="form-section">
                        <div class="form-section-title"><span class="fst-icon fst-purple"><i class="fas fa-money-bill-wave"></i></span> التسعير</div>
                        <div class="form-grid">
                            <div class="form-group">
                                <label class="form-label">سعر البيع (ر.س) <span class="req">*</span></label>
                                <input type="number" step="0.01" name="price" class="form-input" id="prodPrice" placeholder="0.00" min="0" required style="direction:ltr;text-align:right;">
                            </div>
                            <div class="form-group">
                                <label class="form-label">إجمالي القيمة التقديرية</label>
                                <input type="text" class="form-input" id="totalValue" readonly style="background:#f8fafc;color:var(--text-muted);cursor:default;direction:ltr;text-align:right;" placeholder="الكمية × السعر">
                                <div class="form-hint"><i class="fas fa-calculator"></i> تُحسب تلقائياً (الكمية × السعر)</div>
                            </div>
                        </div>
                    </div>

                    <div class="form-actions">
                        <button type="submit" class="btn-submit" id="btnSubmit"><i class="fas fa-save"></i> حفظ وإضافة المنتج</button>
                        <a href="<?php echo URL_ROOT; ?>/product/index" class="btn-cancel">إلغاء</a>
                    </div>
                </form>
            </div>
    ...
    <script>
        // حساب إجمالي قيمة المنتج التلقائي
        ...
        function calcTotal() {
            ...
            totalInput.value = total > 0 ? total.toLocaleString('ar-SA', {
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (!name.value.trim()) {
                ...
                name.parentNode.appendChild(makeErr('اسم المنتج مطلوب'));
                valid = false;
            }

            if (!sku.value.trim()) {
                sku.classList.add('has-error');
                sku.parentNode.appendChild(makeErr('رمز الـ SKU مطلوب'));
                valid = false;
            }
