...
$pageTitle = $data['title'] ?? 'الجزاءات والمخالفات';
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .summary-card:nth-child(3) .sc-icon { background: var(--accent-light); color: var(--accent); }
        ...
        .toolbar-right h3 i { color: var(--danger); }
        
        .btn-add { display: inline-flex; align-items: center; gap: 8px; padding: 10px 22px; background: linear-gradient(135deg, var(--danger), #dc2626); color: #fff; border: none; border-radius: var(--radius-sm); font-family: 'Cairo', sans-serif; font-size: 13px; font-weight: 600; cursor: pointer; text-decoration: none; transition: all 0.25s; box-shadow: 0 2px 10px rgba(239, 68, 68, 0.2); }
        .btn-add:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(239, 68, 68, 0.3); }
        ...
        .date-badge { font-family: monospace; font-size: 12px; background: var(--page-bg); padding: 4px 8px; border-radius: 6px; border: 1px solid var(--border); color: var(--text-dark); font-weight: 600;}
        .amount-val { font-weight: 800; color: var(--danger); font-variant-numeric: tabular-nums; direction: ltr; display: inline-block;}
        ...
        .badge-deduction { background: var(--danger-light); color: #dc2626; }
        ...
        .btn-del { color: var(--text-muted); } .btn-del:hover { background: var(--danger-light); border-color: var(--danger); color: var(--danger); }
        ...
        <div class="page-body">
            ...
            <div class="summary-grid">
                <div class="summary-card">
                    ...
                    <div>
                        <div class="sc-label">إجمالي المخالفات</div>
                        <div class="sc-value"><?php echo count($sanctions); ?></div>
                    </div>
                </div>
                <div class="summary-card">
                    <div class="sc-icon"><i class="fas fa-money-bill-transfer"></i></div>
                    <div>
                        <div class="sc-label">إجمالي الخصومات (مالية)</div>
                        <div class="sc-value" style="color:var(--danger);"><?php echo number_format($totalDeductions, 2); ?> <span class="sc-unit">ر.س</span></div>
                    </div>
                </div>
                <div class="summary-card">
                    <div class="sc-icon"><i class="fas fa-triangle-exclamation"></i></div>
                    <div>
                        <div class="sc-label">إنذارات شفهية / كتابية</div>
                        <div class="sc-value" style="color:var(--accent);"><?php echo $warningsCount; ?></div>
                    </div>
                </div>
            </div>
            ...
            <div class="toolbar">
                <div class="toolbar-right">
                    <h3><i class="fas fa-file-signature"></i> سجل الجزاءات الإدارية</h3>
                </div>
                <div>
                    <a href="<?php echo URL_ROOT; ?>/sanction/create" class="btn-add">
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th>الموظف</th>
                                <th>نوع الجزاء</th>
                                <th style="text-align:left;">مبلغ الخصم</th>
                        ...
                        <tbody>
                            <?php foreach ($sanctions as $s) : 
                                $typeClass = $s->type === 'deduction' ? 'badge-deduction' : 'badge-warning';
                                $typeLabel = $s->type === 'deduction' ? 'خصم مالي' : 'إنذار';
                                $typeIcon = $s->type === 'deduction' ? 'money-bill' : 'triangle-exclamation';
                            ?>
                            <tr>
                                <td style="color:var(--text-muted);font-weight:600;font-size:12px;"><?php echo $s->id; ?></td>
                                <td class="emp-name">
                                    <i class="fas fa-user-circle" style="color:var(--text-muted);margin-left:4px;"></i>
                                    <?php echo htmlspecialchars($s->employee_name ?? '—'); ?>
                                </td>
                                <td>
                                    <span class="badge <?php echo $typeClass; ?>">
                                        <i class="fas fa-<?php echo $typeIcon; ?>"></i> <?php echo $typeLabel; ?>
                                    </span>
                                </td>
                                <td style="text-align:left;">
                                    <?php if ($s->type === 'deduction' && $s->amount > 0) : ?>
                                        <span class="amount-val">-<?php echo number_format($s->amount, 2); ?></span>
                                    <?php else : ?>
                                        <span style="color:var(--text-muted);">—</span>
                                    <?php endif; ?>
                                </td>
                                <td><span class="date-badge"><i class="far fa-calendar-alt"></i> <?php echo date('Y-m-d', strtotime($s->date)); ?></span></td>
                                <td><span style="font-size:12px;color:var(--text-muted);"><?php echo htmlspecialchars(mb_substr($s->reason, 0, 30)) . '...'; ?></span></td>
                                <td style="font-size:12px; font-weight:600;"><i class="fas fa-user-gear" style="color:var(--text-muted);margin-left:4px;"></i><?php echo htmlspecialchars($s->created_by_name ?? 'النظام'); ?></td>
                                <td style="text-align:center;">
                                    <div style="display:flex;align-items:center;justify-content:center;">
                                        <form method="POST" action="<?php echo URL_ROOT; ?>/sanction/delete/<?php echo $s->id; ?>" style="display:inline;" onsubmit="return confirm('تأكيد حذف وإلغاء هذا الجزاء؟')">
                                            <button type="submit" class="act-btn btn-del" title="إلغاء وحذف"><i class="fas fa-trash"></i></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td colspan="8">
                                    <div class="empty-state">
                                        ...
                                        <h4>لا توجد جزاءات مسجلة</h4>
                                        <p>سجل الموظفين نظيف! لم يتم توقيع أي جزاءات أو مخالفات إدارية.</p>
                                    </div>
                                </td>
                            </tr>
                            <?php endif; ?>
