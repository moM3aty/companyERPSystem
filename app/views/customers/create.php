...
// app/views/customers/create.php
 ...
 $currentUrl = $_GET['url'] ?? 'customer/create';
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-header-card::after { content: ''; position: absolute; width: 160px; height: 160px; background: rgba(255,255,255,0.04); border-radius: 50%; bottom: -60px; right: 60px; }
...
<body>
        ...
        <header class="topbar">
            <div style="display:flex;align-items:center;gap:16px;">
                ...
                <div>
                    ...
                    <div class="breadcrumb"><a href="<?php echo URL_ROOT; ?>/dashboard">الرئيسية</a><i class="fas fa-chevron-left" style="font-size:9px;"></i><a href="<?php echo URL_ROOT; ?>/customer/index">العملاء</a><i class="fas fa-chevron-left" style="font-size:9px;"></i><span>إضافة عميل</span></div>
                </div>
            </div>
        </header>
        <div class="page-body">
            <?php if ($flash) : ?>
                <div class="flash-msg flash-<?php echo $flash['type']; ?>"><i class="fas fa-<?php echo $flash['type'] === 'success' ? 'circle-check' : 'circle-xmark'; ?>"></i><?php echo htmlspecialchars($flash['message']); ?></div>
            <?php endif; ?>
            <div class="form-header-card">
                <h2><i class="fas fa-user-plus" style="margin-left:8px;"></i> إضافة عميل جديد</h2>
                <p>أدخل بيانات العميل — سيتم ربطه بالفواتير والمدفوعات تلقائياً</p>
            </div>
            <div class="form-card">
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" class="form-input" id="custName" placeholder="مثال: شركة الأمل للتجارة" required>
                                <div class="form-hint"><i class="fas fa-info-circle"></i> الاسم كما سيظهر في الفواتير</div>
                            </div>
                            <div class="form-group">
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <div class="form-hint"><i class="fas fa-envelope"></i> سيُرسل إليه الفواتير تلقائياً</div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group full">
                                ...
                                <div class="form-hint"><i class="fas fa-location-dot"></i> عنوان الفاتورة والتوصيل</div>
                            </div>
                        </div>
                    </div>
                    <div class="form-section">
                        ...
                        <div class="form-group full">
                            <textarea name="notes" class="form-input" id="custNotes" placeholder="أي ملاحظات إضافية عن العميل..." rows="3"></textarea>
                            <div class="form-hint"><i class="fas fa-info-circle"></i> ملاحظات داخلية لا تظهر للعميل</div>
                        </div>
                    </div>
                    <div class="form-actions">
                        <button type="submit" class="btn-submit" id="btnSubmit"><i class="fas fa-check-circle"></i> حفظ العميل</button>
    ...
    <script>
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (!n.value.trim() || n.value.trim().length < 3) { n.style.borderColor = 'var(--danger)'; v = false; }
            if (em.value && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(em.value)) { em.style.borderColor = 'var(--danger)'; v = false; }
            if (!v) { e.preventDefault(); n.scrollIntoView({ behavior: 'smooth', block: 'center' }); }
            else { btn.disabled = true; btn.innerHTML = '<i class="fas fa-circle-notch fa-spin"></i> جاري الحفظ...'; }
        });
