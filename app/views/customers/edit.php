...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-header-card .fhc-icon {
            width: 48px; height: 48px; border-radius: 12px;
            background: rgba(245,158,11,0.15);
            display: flex; align-items: center; justify-content: center;
            font-size: 20px; color: var(--accent);
        }
        .form-header-card h2 { font-size: 20px; font-weight: 700; }
        .form-header-card p { font-size: 12px; color: #94a3b8; position: relative; z-index: 2; }
        ...
        .fhc-cust-badge .fb-sku { font-family: monospace; direction: ltr; font-size: 13px; color: var(--accent); font-weight: 700; }
        .fhc-cust-badge .fb-id { font-size: 11px; color: #94a3b8; }

        .info-bar {
            display: flex; align-items: center; gap: 20px;
            padding: 16px 24px; background: var(--card-bg);
            border-radius: var(--radius); border: 1px solid var(--border);
            margin-bottom: 20px; animation: fadeUp 0.5s ease 0.05s both;
            flex-wrap: wrap;
        }
        .ib-item { display: flex; align-items: center; gap: 8px; font-size: 13px; color: var(--text-body); }
        .ib-item i { color: var(--text-muted); font-size: 12px; width: 16px; }
        .ib-item strong { color: var(--text-dark); }
        .ib-divider { width: 1px; height: 24px; background: var(--border); }
        ...
        @media (max-width: 768px) {
            ...
            .form-header-card { padding: 24px 20px; } .form-header-card .fhc-top { flex-direction: column; align-items: flex-start; }
            .info-bar { flex-direction: column; gap: 8px; padding: 12px 16px; }
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="page-body">
                ...
                <div class="fhc-top">
                    <div class="fhc-title">
                        ...
                        <div>
                            ...
                            <p>قم بتحديث البيانات المطلوبة ثم احفظ</p>
                        </div>
                    </div>
                    <div class="fhc-cust-badge">
                        <div>
                            <div class="fb-sku">ID-<?php echo str_pad($c->id, 5, '0', STR_PAD_LEFT); ?></div>
                            <div class="fb-id">معرّف العميل</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="info-bar">
                <div class="ib-item"><i class="fas fa-user"></i> الاسم: <strong><?php echo htmlspecialchars($c->name); ?></strong></div>
                <div class="ib-divider"></div>
                <div class="ib-item"><i class="fas fa-receipt"></i> الفواتير: <strong><?php echo (int)($c->invoice_count ?? 0); ?></strong></div>
                <div class="ib-divider"></div>
                <div class="ib-item"><i class="fas fa-coins"></i> المشتريات: <strong style="direction:ltr;display:inline-block;"><?php echo number_format($c->total_purchases ?? 0, 2); ?></strong> ر.س</div>
                <div class="ib-divider"></div>
                <div class="ib-item"><i class="fas fa-wallet"></i> الرصيد: <strong style="color:<?php echo $c->balance > 0 ? 'var(--danger)' : 'var(--success)'; ?>;direction:ltr;display:inline-block;"><?php echo number_format($c->balance, 2); ?></strong> ر.س</div>
            </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="phone" class="form-input" id="custPhone" value="<?php echo htmlspecialchars($c->phone ?? ''); ?>" style="direction:ltr;text-align:right;">
                            </div>
                            <div class="form-group full">
                                <label class="form-label">العنوان</label>
                                <textarea name="address" class="form-input" id="custAddress" rows="2"><?php echo htmlspecialchars($c->address ?? ''); ?></textarea>
                            </div>
                        </div>
                    </div>

                    <div class="form-section">
                        <div class="form-section-title"><span class="fst-icon fst-amber"><i class="fas fa-sticky-note"></i></span> ملاحظات</div>
                        <div class="form-group full">
                            <textarea name="notes" class="form-input" id="custNotes" rows="3"><?php echo htmlspecialchars($c->notes ?? ''); ?></textarea>
                        </div>
                    </div>

                    <div class="form-actions">
                        <button type="submit" class="btn-submit" id="btnSubmit"><i class="fas fa-save"></i> حفظ التعديلات</button>
                        <a href="<?php echo URL_ROOT; ?>/customer/index" class="btn-cancel"><i class="fas fa-arrow-right"></i> رجوع للقائمة</a>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (!name.value.trim() || name.value.trim().length < 3) {
                ...
                err.innerHTML = '<i class="fas fa-exclamation-circle"></i> اسم العميل مطلوب (3 أحرف على الأقل)';
