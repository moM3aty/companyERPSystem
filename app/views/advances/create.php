...
$pageTitle = $data['title'] ?? 'طلب سلفة جديدة';
$employees = $data['employees'] ?? [];
$flash = $data['flash'] ?? null;
$currentUrl = 'advance/index';
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-section-title .fst-icon { width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 14px; flex-shrink: 0; background: var(--accent-light); color: var(--accent); }
        ...
        .salary-hint { font-size: 12px; color: var(--text-muted); background: var(--page-bg); padding: 8px 12px; border-radius: 8px; margin-top: 8px; display: none; align-items: center; gap: 8px; font-weight: 600; border: 1px dashed var(--border);}
        .warning-hint { font-size: 11px; color: var(--danger); margin-top: 6px; display: none; align-items: center; gap: 4px; animation: fadeUp 0.3s ease;}
...
<body>
    ...
    <div class="main-content">
            ...
            <div style="display:flex;align-items:center;gap:16px;">
                ...
                <div>
                    ...
                    <div class="breadcrumb">
                        ...
                        <a href="<?php echo URL_ROOT; ?>/advance/index">السلف والعهد</a>
                        <i class="fas fa-chevron-left" style="font-size:9px;"></i>
                        <span>طلب سلفة</span>
                    </div>
                </div>
        ...
        <div class="page-body">
            ...
            <div class="form-header-card">
                <h2><i class="fas fa-hand-holding-dollar"></i> نموذج تقديم طلب سلفة</h2>
                <p>يتم تحويل الطلب للإدارة المالية للمراجعة والاعتماد. سيتم الخصم تلقائياً في الشهر المحدد للخصم ضمن مسير الرواتب.</p>
            </div>

            <div class="form-card">
                <form action="<?php echo URL_ROOT; ?>/advance/create" method="POST" id="advForm" novalidate>
                    
                    <div class="form-section">
                        <div class="form-section-title"><span class="fst-icon"><i class="fas fa-user-tag"></i></span> بيانات الموظف والطلب</div>
                        <div class="form-grid">
                            <div class="form-group full">
                                <label class="form-label">الموظف طالب السلفة <span class="req">*</span></label>
                                ...
                                <div id="salaryHint" class="salary-hint">
                                    <i class="fas fa-wallet"></i> الراتب الأساسي: <strong id="salaryVal" style="font-family:monospace;direction:ltr;color:var(--text-dark);">0</strong> ر.س
                                </div>
                            </div>
                            
                            <div class="form-group">
                                <label class="form-label">مبلغ السلفة المطلوب (ر.س) <span class="req">*</span></label>
                                <input type="number" name="amount" id="amountInput" class="form-input" min="1" step="0.01" placeholder="مثال: 1500" required style="direction:ltr; text-align:right;">
                                <div id="warningHint" class="warning-hint"><i class="fas fa-exclamation-triangle"></i> تنبيه: السلفة تتجاوز راتب الموظف الأساسي!</div>
                            </div>

                            <div class="form-group">
                                <label class="form-label">تاريخ تقديم الطلب <span class="req">*</span></label>
                                <input type="date" name="date" class="form-input" value="<?php echo date('Y-m-d'); ?>" required>
                            </div>

                            <div class="form-group">
                                <label class="form-label">شهر الخصم (الاستقطاع) <span class="req">*</span></label>
                                <select name="deduction_month" class="form-input" required>
                                    <?php 
                                        $currentMonth = date('n');
                            ...
                            <div class="form-group">
                                <label class="form-label">سنة الخصم <span class="req">*</span></label>
                                <select name="deduction_year" class="form-input" required>
                                    <?php 
                                        $currentYear = date('Y');
                            ...
                            <div class="form-group full">
                                ...
                                <input type="text" name="reason" class="form-input" placeholder="مثال: ظروف صحية طارئة، صيانة سيارة...">
                            </div>
                        </div>
                    </div>
                    
                    <div class="form-actions">
                        <button type="submit" class="btn-submit" id="btnSubmit"><i class="fas fa-paper-plane"></i> تقديم الطلب للاعتماد</button>
                        <a href="<?php echo URL_ROOT; ?>/advance/index" class="btn-cancel">إلغاء</a>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        const warningHint = document.getElementById('warningHint');
        ...
        empSelect.addEventListener('change', function() {
            ...
            if(selectedOpt.value) {
                currentSalary = parseFloat(selectedOpt.dataset.salary) || 0;
                salaryVal.textContent = currentSalary.toLocaleString('ar-SA', {minimumFractionDigits: 2});
        ...
        // التحقق من أن السلفة لا تتجاوز الراتب بشكل مبالغ فيه (تحذير فقط)
        amountInput.addEventListener('input', checkAmount);

        function checkAmount() {
            if (currentSalary > 0 && amountInput.value) {
                if (parseFloat(amountInput.value) > currentSalary) {
                    warningHint.style.display = 'flex';
                } else {
                    warningHint.style.display = 'none';
                }
            }
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (!valid) {
                ...
                btnSubmit.innerHTML = '<i class="fas fa-circle-notch fa-spin"></i> جاري إرسال الطلب...';
            }
        });
