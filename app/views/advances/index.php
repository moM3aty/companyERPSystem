...
$pageTitle = $data['title'] ?? 'سجل السلف والعهد';
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .date-badge { font-family: monospace; font-size: 12px; background: var(--page-bg); padding: 4px 8px; border-radius: 6px; border: 1px solid var(--border); color: var(--text-muted);}
        .amount-val { font-weight: 800; color: var(--danger); font-variant-numeric: tabular-nums; direction: ltr; display: inline-block;}
        .deduct-month { font-size: 11px; font-weight: 700; background: var(--info-light); color: var(--info-dark); padding: 2px 8px; border-radius: 6px; }
        ...
        .badge-deducted { background: var(--purple-light); color: var(--purple); }
        ...
        <div class="page-body">
            ...
            <div class="toolbar">
                <div class="toolbar-right">
                    <h3><i class="fas fa-hand-holding-dollar"></i> سجل سلف الموظفين</h3>
                </div>
                <div>
                    <a href="<?php echo URL_ROOT; ?>/advance/create" class="btn-add">
                        <i class="fas fa-plus"></i> طلب سلفة جديدة
                    </a>
                </div>
            </div>
                    ...
                    <table>
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>الموظف</th>
                                <th style="text-align:left;">المبلغ المطلوب</th>
                                <th>تاريخ الطلب</th>
                                <th style="text-align:center;">شهر الخصم (الاستقطاع)</th>
                                <th style="text-align:center;">الحالة</th>
                                <?php if ($isAdmin) : ?><th style="text-align:center;">مراجعة الإدارة</th><?php endif; ?>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($advances as $adv) : 
                                ...
                                $statusIcon = match($adv->status) {
                                    'approved' => 'check',
                                    'rejected' => 'xmark',
                                    'deducted' => 'check-double',
                                    default => 'clock'
                                };
                            ?>
                            <tr>
                                <td style="color:var(--text-muted);font-weight:600;font-size:12px;"><?php echo $adv->id; ?></td>
                                <td>
                                    <span class="emp-name"><?php echo htmlspecialchars($adv->employee_name ?? '—'); ?></span>
                                    <span style="font-size:11px;color:var(--text-muted);"><?php echo htmlspecialchars($adv->reason ?? 'بدون سبب'); ?></span>
                                </td>
                                <td style="text-align:left;"><span class="amount-val"><?php echo number_format($adv->amount, 2); ?> ر.س</span></td>
                                <td><span class="date-badge"><?php echo date('Y-m-d', strtotime($adv->date)); ?></span></td>
                                <td style="text-align:center;">
                                    <span class="deduct-month"><?php echo $adv->deduction_month . ' / ' . $adv->deduction_year; ?></span>
                                </td>
                                <td style="text-align:center;">
                                ...
                                <td style="text-align:center;">
                                    <?php if ($adv->status === 'pending') : ?>
                                        <form method="POST" action="<?php echo URL_ROOT; ?>/advance/approve/<?php echo $adv->id; ?>" style="display:inline;" onsubmit="return confirm('تأكيد الموافقة على السلفة واعتماد خصمها؟');">
                                            <button type="submit" class="act-btn btn-success" title="اعتماد وموافقة"><i class="fas fa-check"></i></button>
                                        </form>
                                        <form method="POST" action="<?php echo URL_ROOT; ?>/advance/reject/<?php echo $adv->id; ?>" style="display:inline;" onsubmit="return confirm('هل أنت متأكد من رفض السلفة؟');">
                                            <button type="submit" class="act-btn btn-danger" title="رفض الطلب"><i class="fas fa-times"></i></button>
                                        </form>
                                    <?php else: ?>
                                        <span style="color:var(--text-muted); font-size:11px; font-weight:600;"><i class="fas fa-lock"></i> مقفل</span>
                                    <?php endif; ?>
                                </td>
                                <?php endif; ?>
                            </tr>
                            ...
                            <tr>
                                <td colspan="<?php echo $isAdmin ? '7' : '6'; ?>">
                                    <div class="empty-state">
                                        ...
                                        <h4>لا توجد طلبات سلف</h4>
                                        <p>لم يتم تقديم أي طلبات سلف أو عهد من قبل الموظفين.</p>
                                    </div>
                                </td>
                            </tr>
                            <?php endif; ?>
