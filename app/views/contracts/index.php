...
$pageTitle = $data['title'] ?? 'إدارة العقود والتنبيهات';
$contracts = $data['contracts'] ?? [];
$flash = $data['flash'] ?? null;
$currentUrl = 'contract/index';
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .flash-msg.flash-info { background: var(--info-light); color: #0e7490; border-color: rgba(6,182,212,0.2); }
        ...
        .summary-card:nth-child(1) .sc-icon { background: var(--success-light); color: var(--success); }
        .summary-card:nth-child(2) .sc-icon { background: var(--accent-light); color: var(--accent); }
        .summary-card:nth-child(3) .sc-icon { background: var(--danger-light); color: var(--danger); }
        ...
        .cont-title { font-weight: 700; color: var(--text-dark); display: block; margin-bottom: 2px;}
        .cont-party { font-size: 12px; color: var(--text-muted); display: flex; align-items: center; gap: 6px; }
        .cont-party i { font-size: 10px; }
        ...
        .badge-pending { background: var(--border); color: var(--text-body); }

        .date-val { font-family: monospace; font-size: 12px; font-weight: 600; color: var(--text-dark); }
        
        .val-amount { font-weight: 700; color: var(--text-dark); font-variant-numeric: tabular-nums; direction: ltr; display: inline-block;}
        ...
        <div class="page-body">
            ...
            <div class="summary-grid">
                <div class="summary-card">
                    <div class="sc-icon"><i class="fas fa-file-signature"></i></div>
                    <div>
                        <div class="sc-label">عقود سارية</div>
                        <div class="sc-value"><?php echo $activeContracts; ?></div>
                    </div>
                </div>
                <div class="summary-card">
                    <div class="sc-icon"><i class="fas fa-bell"></i></div>
                    <div>
                        <div class="sc-label">عقود تنتهي قريباً (30 يوم)</div>
                        <div class="sc-value" style="color:var(--accent);"><?php echo $expiringContracts; ?></div>
                    </div>
                </div>
                <div class="summary-card">
                    <div class="sc-icon"><i class="fas fa-calendar-times"></i></div>
                    <div>
                        <div class="sc-label">عقود منتهية الصلاحية</div>
                        <div class="sc-value" style="color:var(--danger);"><?php echo $expiredContracts; ?></div>
                    </div>
                </div>
            </div>
            ...
            <div class="toolbar">
                <div class="toolbar-right">
                    <div class="search-box">
                        <input type="text" id="searchInput" placeholder="ابحث باسم العقد أو الطرف المعني..." autocomplete="off">
                        <i class="fas fa-search"></i>
                    </div>
                </div>
                <div>
                    <a href="<?php echo URL_ROOT; ?>/contract/create" class="btn-add">
                        ...
                        <tbody>
                            <?php foreach ($contracts as $c) : 
                                ...
                                $statusLabel = match($c->display_status ?? $c->status) {
                                    ...
                                    'pending' => 'معلق / غير مفعل',
                                    default => $c->status
                                };
                                $partyIcon = $c->party_type === 'customer' ? 'fa-user-tie' : 'fa-truck-field';
                                $partyLabel = $c->party_type === 'customer' ? 'عميل' : 'مورد';
                            ?>
                            <tr class="search-row" data-search="<?php echo htmlspecialchars($c->title . ' ' . $c->party_name); ?>">
                                <td style="color:var(--text-muted);font-weight:600;font-size:12px;"><?php echo $c->id; ?></td>
                                <td>
                                    <span class="cont-title"><?php echo htmlspecialchars($c->title); ?></span>
                                    <span class="cont-party"><i class="fas <?php echo $partyIcon; ?>"></i> <?php echo $partyLabel . ': ' . htmlspecialchars($c->party_name ?? 'غير محدد'); ?></span>
                                </td>
                                <td><span class="date-val"><?php echo $c->start_date; ?></span></td>
                                <td><span class="date-val" style="<?php echo ($c->display_status === 'expired') ? 'color:var(--danger);' : ''; ?>"><?php echo $c->end_date; ?></span></td>
                                <td><span class="val-amount"><?php echo number_format($c->value, 2); ?> ر.س</span></td>
                                <td>
                                ...
                                <td>
                                    <div class="actions-cell">
                                        <button class="act-btn btn-view" title="عرض تفاصيل العقد (قيد التطوير)" onclick="alert('واجهة عرض تفاصيل العقد ستكون متاحة في التحديث القادم')"><i class="fas fa-eye"></i></button>
                                        <form method="POST" action="<?php echo URL_ROOT; ?>/contract/delete/<?php echo $c->id; ?>" style="display:inline;" onsubmit="return confirm('هل أنت متأكد من حذف هذا العقد نهائياً؟')">
                                            <button type="submit" class="act-btn btn-del" title="حذف العقد"><i class="fas fa-trash"></i></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td colspan="7">
                                    <div class="empty-state">
                                        ...
                                        <p>قم بإضافة بيانات العقود الخاصة بالعملاء أو الموردين ليتم تنبيهك قبل انتهائها.</p>
                                        <a href="<?php echo URL_ROOT; ?>/contract/create" class="btn-add" style="display:inline-flex;">
                                            <i class="fas fa-plus"></i> تسجيل عقد
                                        </a>
                                    </div>
                                </td>
                            </tr>
