...
/* ألوان كل دور */
.role-hr .role-icon { color: #a855f7; }
.role-radio:checked + .role-hr { border-color: #a855f7; background: #faf5ff; }
.role-radio:checked + .role-hr::before { color: #a855f7; }

.role-sales .role-icon { color: #10b981; }
.role-radio:checked + .role-sales { border-color: #10b981; background: #ecfdf5; }
.role-radio:checked + .role-sales::before { color: #10b981; }

.role-acc .role-icon { color: #3b82f6; }
.role-radio:checked + .role-acc { border-color: #3b82f6; background: #eff6ff; }
.role-radio:checked + .role-acc::before { color: #3b82f6; }

.role-mgr .role-icon { color: #f59e0b; }
.role-radio:checked + .role-mgr { border-color: #f59e0b; background: #fffbeb; }
.role-radio:checked + .role-mgr::before { color: #f59e0b; }

.role-admin .role-icon { color: #ef4444; }
.role-radio:checked + .role-admin { border-color: #ef4444; background: #fef2f2; }
.role-radio:checked + .role-admin::before { color: #ef4444; }

.role-editor .role-icon { color: #0ea5e9; }
.role-radio:checked + .role-editor { border-color: #0ea5e9; background: #f0f9ff; }
.role-radio:checked + .role-editor::before { color: #0ea5e9; }

.role-viewer .role-icon { color: #64748b; }
.role-radio:checked + .role-viewer { border-color: #64748b; background: #f8fafc; }
.role-radio:checked + .role-viewer::before { color: #64748b; }
...
<div class="card" style="max-width: 1000px; margin: 0 auto;">
    <div class="card-header bg-light">
        <h3 class="card-title text-dark"><i class="fas fa-user-plus text-success"></i> إضافة مستخدم جديد للنظام</h3>
    </div>
    ...
    <form action="<?php echo URLROOT; ?>/user/create" method="POST">
        <div class="card-body">
            ...
            <h5 class="fw-bold mb-3 text-dark border-bottom pb-2">1. بيانات الدخول الأساسية</h5>
            <div class="form-grid mb-4">
                <div class="form-group">
                    <label class="form-label">اسم المستخدم الكامل <span class="required">*</span></label>
                    <input type="text" name="name" class="form-control" value="<?php echo htmlspecialchars($_POST['name'] ?? ''); ?>" required placeholder="مثال: أحمد محمد">
                </div>
                <div class="form-group">
                    <label class="form-label">البريد الإلكتروني <span class="required">*</span></label>
                    <input type="email" name="email" class="form-control" value="<?php echo htmlspecialchars($_POST['email'] ?? ''); ?>" required style="direction:ltr; text-align:right;" placeholder="email@company.com">
                </div>
                <div class="form-group full-width">
                    <label class="form-label">تعيين كلمة المرور <span class="required">*</span></label>
                    <input type="password" name="password" class="form-control" required style="direction:ltr; text-align:right;">
                </div>
            </div>

            <h5 class="fw-bold mb-2 text-dark border-bottom pb-2 mt-4">2. الصلاحيات وتعيين الدور (RBAC) <span class="required">*</span></h5>
            <p class="text-muted" style="font-size: 12px;">اختر بطاقة الصلاحيات المناسبة لتحديد ما يمكن لهذا المستخدم رؤيته والتعديل عليه في النظام.</p>
        ...
        <div class="card-footer d-flex gap-3 bg-light mt-0">
            <button type="submit" class="btn btn-primary"><i class="fas fa-user-check"></i> إنشاء المستخدم</button>
