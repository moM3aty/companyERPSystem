...
$pageTitle = $data['title'] ?? 'تعديل بيانات المورد';
...
<head>
    ...
    <title><?php echo $pageTitle; ?> — <?php echo htmlspecialchars($s->name); ?></title>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-header-card::before { content: ''; position: absolute; width: 200px; height: 200px; background: rgba(245,158,11,0.1); border-radius: 50%; top: -80px; left: -40px; }
        .fhc-icon { width: 56px; height: 56px; border-radius: 14px; background: rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: center; font-size: 24px; color: var(--accent-light); position: relative; z-index: 2;}
    ...
    <div class="main-content">
        ...
        <div class="page-body">
            ...
            <div class="form-header-card">
                <div class="fhc-icon"><i class="fas fa-truck-pen"></i></div>
                <div class="fhc-text">
                    <h2>تعديل بيانات المورد</h2>
                    <p>المعرف المرجعي للمورد: #<?php echo $s->id; ?></p>
                </div>
            </div>

            <div class="form-card">
                <form action="<?php echo URL_ROOT; ?>/supplier/edit/<?php echo $s->id; ?>" method="POST" id="supplierForm" novalidate>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" class="form-input" id="supplierName" value="<?php echo htmlspecialchars($s->name); ?>" required>
                            </div>
                            <div class="form-group">
                                <label class="form-label">نوع المورد</label>
                                <select name="type" class="form-input" id="supplierType">
                                    <option value="company" <?php echo $s->type === 'company' ? 'selected' : ''; ?>>شركة / منشأة</option>
                                    <option value="individual" <?php echo $s->type === 'individual' ? 'selected' : ''; ?>>فرد</option>
                                </select>
                            </div>
                            <div class="form-group full">
                                <label class="form-label">الشخص المسؤول (جهة الاتصال)</label>
                                <input type="text" name="contact_person" class="form-input" value="<?php echo htmlspecialchars($s->contact_person ?? ''); ?>">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <input type="email" name="email" class="form-input" id="supplierEmail" value="<?php echo htmlspecialchars($s->email ?? ''); ?>" style="direction:ltr;text-align:right;">
                            </div>
                            <div class="form-group">
                                <label class="form-label">رقم الهاتف</label>
                                <input type="text" name="phone" class="form-input" value="<?php echo htmlspecialchars($s->phone ?? ''); ?>" style="direction:ltr;text-align:right;">
                            </div>
                            <div class="form-group full">
                                <label class="form-label">العنوان الفعلي</label>
                                <textarea name="address" class="form-input" rows="2"><?php echo htmlspecialchars($s->address ?? ''); ?></textarea>
                            </div>
                        </div>
                    </div>
                    
                    <div class="form-section">
                        <div class="form-section-title"><span class="fst-icon fst-amber"><i class="fas fa-sticky-note"></i></span> ملاحظات إضافية</div>
                        <div class="form-group full">
                            <textarea name="notes" class="form-input" rows="3"><?php echo htmlspecialchars($s->notes ?? ''); ?></textarea>
                        </div>
                    </div>
                    
                    <div class="form-actions">
                        <button type="submit" class="btn-submit" id="btnSubmit"><i class="fas fa-save"></i> حفظ التعديلات</button>
                        <a href="<?php echo URL_ROOT; ?>/supplier/index" class="btn-cancel"><i class="fas fa-arrow-right"></i> إلغاء</a>
                    </div>
                </form>
            </div>
