...
$pageTitle = $data['title'] ?? 'المبيعات والفواتير';
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .inv-num { font-family: monospace; font-size: 13px; font-weight: 700; color: var(--primary-dark); background: var(--primary-light); padding: 4px 10px; border-radius: 6px; direction: ltr; display: inline-block; }
        .cust-name { font-weight: 600; color: var(--text-dark); display: flex; align-items: center; gap: 8px;}
        .cust-name i { color: var(--text-muted); font-size: 12px; }
        ...
        .badge-paid { background: var(--success-light); color: #15803d; }
        .badge-unpaid { background: var(--accent-light); color: #b45309; }
        ...
        <div class="page-body">
            ...
            <div class="summary-grid">
                <div class="summary-card">
                    <div class="sc-icon" style="background:var(--success-light);color:var(--success);"><i class="fas fa-coins"></i></div>
                    <div>
                        <div class="sc-label">إجمالي المبيعات</div>
                        <div class="sc-value"><?php echo number_format($totalSales, 2); ?> <span class="sc-unit">ر.س</span></div>
                    </div>
                </div>
                <div class="summary-card">
                    <div class="sc-icon" style="background:var(--info-light);color:var(--info);"><i class="fas fa-receipt"></i></div>
                    <div>
                        <div class="sc-label">عدد الفواتير المصدرة</div>
                        <div class="sc-value"><?php echo number_format($invoiceCount); ?> <span class="sc-unit">فاتورة</span></div>
                    </div>
                </div>
                <div class="summary-card">
                    <div class="sc-icon" style="background:var(--purple-light);color:var(--purple);"><i class="fas fa-chart-line"></i></div>
                    <div>
                        <div class="sc-label">متوسط الفاتورة</div>
                        <div class="sc-value"><?php echo number_format($avgInvoice, 2); ?> <span class="sc-unit">ر.س</span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="toolbar">
                <div class="toolbar-right">
                    <div class="search-box">
                        <input type="text" id="searchInput" placeholder="ابحث برقم الفاتورة أو العميل..." autocomplete="off">
                        <i class="fas fa-search"></i>
                    </div>
                </div>
                <div>
                    <a href="<?php echo URL_ROOT; ?>/sale/create" class="btn-add">
                ...
                <div class="table-wrap">
                    <table>
                        ...
                        <tbody>
                            ...
                            ?>
                            <tr class="search-row" data-search="<?php echo htmlspecialchars($inv->invoice_number . ' ' . ($inv->customer_name ?? $inv->name ?? '')); ?>">
                                <td style="color:var(--text-muted);font-weight:600;font-size:12px;"><?php echo $inv->id; ?></td>
                                ...
                                <td style="text-align:left;">
                                    <span class="amount-val"><?php echo number_format($inv->total_amount, 2); ?> <span class="curr">ر.س</span></span>
                                </td>
                                <td>
                                    <span class="date-val"><i class="far fa-calendar-alt"></i> <?php echo date('Y-m-d H:i', strtotime($inv->created_at)); ?></span>
                                </td>
                                <td>
                                    <?php if ($isPaid) : ?>
                                        <span class="badge badge-paid"><i class="fas fa-check-circle"></i> مدفوعة</span>
                                    <?php else : ?>
                                        <span class="badge badge-unpaid"><i class="fas fa-clock"></i> غير مدفوعة</span>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <div class="actions-cell">
                                        <a href="<?php echo URL_ROOT; ?>/sale/show/<?php echo $inv->id; ?>" class="act-btn btn-view" title="عرض الفاتورة والطباعة"><i class="fas fa-eye"></i></a>
                                    </div>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                            ...
                            <tr>
                                <td colspan="7">
                                    <div class="empty-state">
                                        ...
                                        <h4>لا توجد فواتير مبيعات</h4>
                                        <p>ابدأ بإنشاء فاتورة المبيعات الأولى لعملائك</p>
                                        <a href="<?php echo URL_ROOT; ?>/sale/create" class="btn-add" style="display:inline-flex;">
