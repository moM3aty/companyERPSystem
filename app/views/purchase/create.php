...
$pageTitle = $data['title'] ?? 'إنشاء أمر شراء';
$suppliers = $data['suppliers'] ?? [];
$products = $data['products'] ?? [];
$flash = $data['flash'] ?? null;
$currentUrl = 'purchase/create';
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-header-card { background: linear-gradient(135deg, #334155 0%, #0f172a 100%); border-radius: var(--radius); padding: 28px 32px; color: #fff; margin-bottom: 24px; position: relative; overflow: hidden; animation: fadeUp 0.5s ease both; }
        .form-header-card::before { content: ''; position: absolute; width: 250px; height: 250px; background: rgba(255,255,255,0.03); border-radius: 50%; top: -100px; left: -50px; }
        ...
        .form-header-card h2 i { color: var(--primary); }
        .form-header-card p { font-size: 13px; color: #94a3b8; position: relative; z-index: 2; }
        ...
        .total-value { font-size: 24px; font-weight: 800; color: var(--primary-dark); font-variant-numeric: tabular-nums; direction: ltr; }
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="page-body">
            ...
            <div class="form-header-card">
                <h2><i class="fas fa-cart-arrow-down"></i> إنشاء أمر شراء جديد</h2>
                <p>قم باختيار المورد وإضافة المنتجات التي ترغب في طلبها. سيتم حفظ الأمر بحالة (قيد الانتظار).</p>
            </div>
            ...
            <div class="form-card">
                <form action="<?php echo URL_ROOT; ?>/purchase/create" method="POST" id="purchaseForm">
                    ...
                    <div class="form-section">
                        <div class="form-section-title"><span class="fst-icon fst-teal"><i class="fas fa-truck-field"></i></span> بيانات المورد والتفاصيل</div>
                        <div class="form-grid">
                            <div class="form-group">
                                <label class="form-label">المورد <span class="req">*</span></label>
                                <select name="supplier_id" class="form-input" required id="supplierSelect">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="notes" class="form-input" rows="1" placeholder="شروط التوصيل، تفاصيل الدفع..."></textarea>
                            </div>
                        </div>
                    </div>

                    <div class="form-section">
                        <div class="form-section-title"><span class="fst-icon fst-amber"><i class="fas fa-boxes-stacked"></i></span> المنتجات المطلوبة</div>
                        ...
                        <div class="lines-container" id="items-container">
                            <div class="item-row">
                                ...
                                <input type="number" name="unit_price[]" placeholder="سعر الوحدة (ر.س)" class="form-input price-input" step="0.01" min="0" required style="direction:ltr; text-align:center;">
                        ...
                        <button type="button" onclick="addRow()" class="btn-add-line"><i class="fas fa-plus"></i> إضافة صنف آخر</button>

                        <div class="totals-panel">
                            <span class="total-label">إجمالي أمر الشراء:</span>
                            <span class="total-value" id="grandTotal">0.00 <span style="font-size:14px;color:var(--text-muted);">ر.س</span></span>
                        </div>
                    </div>

                    <div class="form-actions">
                        <button type="submit" class="btn-submit" id="btnSubmit"><i class="fas fa-save"></i> حفظ أمر الشراء</button>
    ...
    <script>
        // تجهيز بيانات المنتجات للمولد الديناميكي
        const productsHtml = `
            <option value="">-- اختر المنتج --</option>
            <?php foreach ($products as $prod) : ?>
                <option value="<?php echo $prod->id; ?>" data-price="<?php echo $prod->price; ?>"><?php echo addslashes(htmlspecialchars($prod->name)); ?></option>
            <?php endforeach; ?>
        `;
        ...
        function addRow() {
            ...
            newRow.innerHTML = `
                ...
                <input type="number" name="unit_price[]" placeholder="سعر الوحدة (ر.س)" class="form-input price-input" step="0.01" min="0" required style="direction:ltr; text-align:center;">
                <div class="subtotal-calc">0.00</div>
                <button type="button" class="btn-remove-line" onclick="removeRow(this)" title="حذف السطر"><i class="fas fa-trash"></i></button>
            `;
        ...
        function calculateGrandTotal() {
            ...
            grandTotalEl.innerHTML = grandTotal.toLocaleString('ar-SA', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + ' <span style="font-size:14px;color:var(--text-muted);">ر.س</span>';
        }
        ...
        function attachEventsToRow(row) {
            ...
            // سحب السعر الافتراضي للمنتج عند اختياره
        ...
        // تفعيل الأحداث للسطر الافتراضي الأول
        ...
        document.getElementById('purchaseForm').addEventListener('submit', function(e) {
            ...
            if(!supplier.value) {
                e.preventDefault();
                supplier.style.borderColor = 'var(--danger)';
