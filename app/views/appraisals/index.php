<?php
// app/views/appraisals/index.php
$pageTitle = $data['title'] ?? 'سجل التقييمات';
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .score-box { display: inline-flex; align-items: center; justify-content: center; width: 38px; height: 38px; border-radius: 50%; font-weight: 800; font-size: 13px; color: #fff;}
        .score-exc { background: var(--success); }
        .score-vgo { background: var(--info); }
        .score-goo { background: var(--primary); }
        .score-acc { background: var(--accent); }
        .score-bad { background: var(--danger); }

        .grade-badge { padding: 4px 10px; border-radius: 6px; font-size: 11px; font-weight: 700; border: 1px solid currentColor;}
        ...
        <div class="page-body">
            ...
            <div class="toolbar">
                <div class="toolbar-right">
                    <h3><i class="fas fa-star-half-stroke"></i> سجل تقييمات الأداء</h3>
                </div>
                <div>
                    <?php if ($isAdmin) : ?>
                    <a href="<?php echo URL_ROOT; ?>/appraisal/create" class="btn-add">
                        ...
                        <tbody>
                            <?php foreach ($appraisals as $app) : 
                                $scoreClass = match(true) {
                                    $app->total_score >= 90 => 'score-exc',
                                    $app->total_score >= 80 => 'score-vgo',
                                    $app->total_score >= 70 => 'score-goo',
                                    $app->total_score >= 60 => 'score-acc',
                                    default => 'score-bad'
                                };
                            ?>
                            <tr>
                                <td><span style="font-family:monospace;font-size:12px;color:var(--text-muted);"><i class="far fa-calendar"></i> <?php echo date('Y-m-d', strtotime($app->evaluation_date)); ?></span></td>
                                <td>
                                    <span class="emp-name"><?php echo htmlspecialchars($app->employee_name ?? '—'); ?></span>
                                    <span class="emp-pos"><?php echo htmlspecialchars($app->position); ?></span>
                                </td>
                                <td style="text-align:center;">
                                    <div class="score-box <?php echo $scoreClass; ?>" title="أداء:<?php echo $app->performance_score;?> | سلوك:<?php echo $app->behavior_score;?> | حضور:<?php echo $app->attendance_score;?>">
                                        <?php echo round($app->total_score); ?>%
                                    </div>
                                </td>
                                <td>
                                    <span class="grade-badge" style="color:var(--<?php echo str_replace('score-', '', $scoreClass) === 'exc' ? 'success' : (str_replace('score-', '', $scoreClass) === 'bad' ? 'danger' : 'info'); ?>);">
                                        <?php echo htmlspecialchars($app->grade); ?>
                                    </span>
                                </td>
                                <td><span style="font-size:12px;color:var(--text-muted);"><?php echo htmlspecialchars(mb_substr($app->comments, 0, 35)) . '...'; ?></span></td>
                                <td><span style="font-size:12px; font-weight:600;"><i class="fas fa-user-gear" style="color:var(--text-muted);margin-left:4px;"></i><?php echo htmlspecialchars($app->evaluator_name); ?></span></td>
                                
                                <?php if ($isAdmin) : ?>
                                <td style="text-align:center;">
                                    <form method="POST" action="<?php echo URL_ROOT; ?>/appraisal/delete/<?php echo $app->id; ?>" style="display:inline;" onsubmit="return confirm('تأكيد حذف هذا التقييم من السجل؟');">
                            ...
                            <tr>
                                <td colspan="<?php echo $isAdmin ? '7' : '6'; ?>">
                                    <div class="empty-state">
                                        ...
                                        <h4>لا توجد تقييمات مسجلة</h4>
                                        <p>قم بإضافة تقييمات دورية لقياس كفاءة أداء فريق العمل.</p>
                                    </div>
                                </td>
                            </tr>
                            <?php endif; ?>
