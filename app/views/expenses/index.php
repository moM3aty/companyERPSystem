...
$pageTitle = $data['title'] ?? 'سجل المصروفات';
$expenses = $data['expenses'] ?? [];
$totalExpenses = $data['total_expenses'] ?? 0;
$isAdmin = $data['is_admin'] ?? false;
$flash = $data['flash'] ?? null;
$currentUrl = 'expense/index';
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-add { display: inline-flex; align-items: center; gap: 8px; padding: 10px 22px; background: linear-gradient(135deg, var(--danger), #b91c1c); color: #fff; border: none; border-radius: var(--radius-sm); font-family: 'Cairo', sans-serif; font-size: 13px; font-weight: 600; cursor: pointer; text-decoration: none; transition: all 0.25s; box-shadow: 0 2px 10px rgba(239,68,68,0.2); }
        .btn-add:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(239,68,68,0.3); }

        .summary-card { background: linear-gradient(135deg, #1e293b, #0f172a); color: #fff; padding: 16px 24px; border-radius: var(--radius); display: flex; align-items: center; gap: 16px; margin-bottom: 24px; box-shadow: var(--shadow-sm); animation: fadeUp 0.5s ease 0.1s both;}
        .summary-card .icon { font-size: 24px; color: var(--danger-light); background: rgba(239, 68, 68, 0.2); width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; border-radius: 12px;}
        .summary-card .val { font-size: 24px; font-weight: 800; font-family: monospace; }
        .summary-card .lbl { font-size: 13px; color: #cbd5e1; }
        ...
        .cat-name { font-weight: 700; color: var(--text-dark); display:block; margin-bottom: 2px;}
        .exp-amount { font-family: monospace; font-size: 15px; font-weight: 800; color: var(--danger); direction: ltr; display: inline-block;}
        ...
        <div class="page-body">
            ...
            <div class="toolbar">
                <div class="toolbar-right">
                    <h3><i class="fas fa-money-bill-transfer"></i> سجل المصروفات التشغيلية</h3>
                </div>
                <div>
                    <a href="<?php echo URL_ROOT; ?>/expense/create" class="btn-add">
            ...
            <div class="summary-card">
                ...
                <div>
                    <div class="val"><?php echo number_format($totalExpenses, 2); ?> <span style="font-size:14px; font-weight:400;">ر.س</span></div>
                    <div class="lbl">إجمالي المصروفات المسجلة</div>
                </div>
            </div>
                ...
                <div class="table-wrap">
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><span style="font-family:monospace;font-size:12px;color:var(--text-muted);"><i class="far fa-calendar"></i> <?php echo date('Y-m-d', strtotime($exp->expense_date)); ?></span></td>
                                <td>
                                    <span class="cat-name"><?php echo htmlspecialchars($exp->category_name ?? 'غير مصنف'); ?></span>
                                    <span style="font-size:11px;color:var(--text-muted);"><i class="fas fa-hashtag"></i> <?php echo htmlspecialchars($exp->reference_no ?? 'بدون مرجع'); ?></span>
                                </td>
                                <td style="text-align:left;">
                                    <span class="exp-amount"><?php echo number_format($exp->amount, 2); ?></span>
                                </td>
                                <td><span style="font-size:12px;color:var(--text-body);"><?php echo htmlspecialchars($exp->notes ?? '—'); ?></span></td>
                                <td><span style="font-size:12px; font-weight:600;"><i class="fas fa-user-pen" style="color:var(--text-muted);margin-left:4px;"></i><?php echo htmlspecialchars($exp->recorded_by_name ?? 'النظام'); ?></span></td>
                                
                                <?php if ($isAdmin) : ?>
                                <td style="text-align:center;">
                                    <form method="POST" action="<?php echo URL_ROOT; ?>/expense/delete/<?php echo $exp->id; ?>" style="display:inline;" onsubmit="return confirm('تأكيد حذف هذا المصروف نهائياً؟');">
                                        <button type="submit" class="act-btn btn-danger" title="حذف"><i class="fas fa-trash"></i></button>
                                    </form>
                                </td>
                                <?php endif; ?>
                            </tr>
                            ...
                            <tr>
                                <td colspan="<?php echo $isAdmin ? '6' : '5'; ?>">
                                    <div class="empty-state">
                                        ...
                                        <p>قم بإضافة المصروفات التشغيلية لضبط ميزانيتك ومتابعة التكاليف.</p>
                                    </div>
                                </td>
                            </tr>
                            <?php endif; ?>
