...
class SalesOrder extends Model {
    ...
    private function autoUpgradeTable() {
        ...
        // 🟢 الحل القاطع: إجبار قاعدة البيانات على تعديل نوع العمود ليكون نصياً (VARCHAR) 🟢
        try {
            $this->db->query("ALTER TABLE `sales_orders` MODIFY COLUMN `status` VARCHAR(50) DEFAULT 'draft'");
            $this->db->execute();
        } catch (Exception $e) {
            // تجاهل الخطأ بصمت إذا لم يكن لدى المستخدم صلاحيات ALTER
        }

        // جدول أصناف أمر البيع
        try {
    ...
    public function createOrder(array $data, array $items): int {
        ...
        $this->db->bind(':odate', !empty($data['order_date']) ? $data['order_date'] : date('Y-m-d'));
        $this->db->bind(':total', $data['total_amount']);
        $this->db->bind(':status', $data['status'] ?? 'draft');
        $this->db->bind(':notes', $data['notes']);
        $this->db->bind(':user', Session::getUserId() ?: 0);
        $this->db->execute();
