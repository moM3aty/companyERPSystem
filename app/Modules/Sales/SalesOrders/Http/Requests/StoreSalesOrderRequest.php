<?php
// Path: app/Modules/Sales/SalesOrders/Http/Requests/StoreSalesOrderRequest.php

declare(strict_types=1);

namespace App\Modules\Sales\SalesOrders\Http\Requests;
...
class StoreSalesOrderRequest
{
    ...
    public function validate(array $data, int $companyId): array
    {
        $rules = [
            ...
            'quotation_id'  => [new Exists($this->db, 'sales_quotations', 'id', $companyId)],
        ...
        if (!is_array($data['items']) || count($data['items']) === 0) {
            throw new ValidationException(['items' => ['Sales Order must contain at least one item.']]);
        }
        ...
        foreach ($data['items'] as $index => $item) {
            ...
            try {
                ...
                throw new ValidationException(["items.{$index}" => $e->getErrors()], "Validation failed for order item at row " . ($index + 1));
            }
        }
