<?php
// Path: app/Modules/Sales/Quotations/Http/Requests/StoreQuotationRequest.php

declare(strict_types=1);

namespace App\Modules\Sales\Quotations\Http\Requests;
...
class StoreQuotationRequest
{
    ...
    public function validate(array $data, int $companyId): array
    {
        $rules = [
            ...
            'quotation_date' => [new Required(), new Date('Y-m-d')],
            'valid_until'    => [new Required(), new Date('Y-m-d')],
        ...
        if (!is_array($data['items']) || count($data['items']) === 0) {
            throw new ValidationException(['items' => ['Quotation must contain at least one item.']]);
        }
        ...
        foreach ($data['items'] as $index => $item) {
            ...
            try {
                ...
                throw new ValidationException(["items.{$index}" => $e->getErrors()], "Validation failed for quotation item at row " . ($index + 1));
            }
        }
