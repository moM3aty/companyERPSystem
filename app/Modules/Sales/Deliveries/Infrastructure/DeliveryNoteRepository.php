<?php
// Path: app/Modules/Sales/Deliveries/Infrastructure/DeliveryNoteRepository.php

declare(strict_types=1);

namespace App\Modules\Sales\Deliveries\Infrastructure;

use App\Core\Database\BaseRepository;
use App\Core\Database\DatabaseManager;
use App\Modules\Sales\Deliveries\Domain\DeliveryNoteRepositoryInterface;

class DeliveryNoteRepository extends BaseRepository implements DeliveryNoteRepositoryInterface
{
    protected string $table = 'sales_delivery_notes';
    protected bool $useTenantScope = true;

    public function __construct(DatabaseManager $db)
    {
        parent::__construct($db);
    }

    public function generateDeliveryNumber(int $companyId): string
    {
        $prefix = 'DEL-' . date('ym') . '-';
        
        $lastRow = $this->newQuery()
            ->select(['delivery_no'])
            ->where('company_id', '=', $companyId)
            ->where('delivery_no', 'LIKE', $prefix . '%')
            ->orderBy('id', 'desc')
            ->first();

        if (!$lastRow) {
            return $prefix . '0001';
        }

        $lastNumber = (int) str_replace($prefix, '', $lastRow['delivery_no']);
        $newNumber = $lastNumber + 1;

        return $prefix . str_pad((string) $newNumber, 4, '0', STR_PAD_LEFT);
    }

    public function bulkInsertItems(int $deliveryId, array $items): void
    {
        if (empty($items)) return;

        $values = [];
        $bindings = [];
        $placeholders = "(?, ?, ?, ?, ?)";

        foreach ($items as $item) {
            $values[] = $placeholders;
            array_push(
                $bindings,
                $deliveryId,
                $item['product_id'],
                $item['warehouse_id'],
                $item['ordered_quantity'] ?? 0.0,
                $item['delivered_quantity']
            );
        }

        $sql = "INSERT INTO sales_delivery_note_items 
                (delivery_note_id, product_id, warehouse_id, ordered_quantity, delivered_quantity) 
                VALUES " . implode(', ', $values);

        $this->db->connection()->insert($sql, $bindings);
    }
}